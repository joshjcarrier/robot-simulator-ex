defmodule Robot do
  ...
  def advance(%Robot{bearing: bearing, position: {x, y}} = robot) when bearing == :north do
    %Robot{robot | position: {x, y + 1}}
  end

  def advance(%Robot{bearing: bearing, position: {x, y}} = robot) when bearing == :south do
    %Robot{robot | position: {x, y - 1}}
  end

  def advance(%Robot{bearing: bearing, position: {x, y}} = robot) when bearing == :east do
    %Robot{robot | position: {x + 1, y}}
  end

  def advance(%Robot{bearing: bearing, position: {x, y}} = robot) when bearing == :west do
    %Robot{robot | position: {x - 1, y}}
  end
...
defmodule RobotSimulator do
  ...
  def create(bearing, position) do
    Robot.create(bearing, position)
  end
